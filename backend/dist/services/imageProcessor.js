...
exports.processWithProgress = exports.processImageGeneration = void 0;
const ne = (s) => {
    const v = (s ?? '').trim();
    return v ? v : null;
};
const parseIntSafe = (s, fallback = 0) => {
    if (typeof s === 'number')
        return s;
    if (!s)
        return fallback;
...
function mapThemeToStoryId(themeSlug) {
    if (!themeSlug)
        return null;
    const mapping = {
...
function buildPrompt(opts) {
    ...
    if (opts.includeTitle && title) {
        basePrompt.push(`Title: Add "${title}" in large, hand-lettered, colorful text at the top-center with high contrast and readability.`);
    }
    basePrompt.push(`Keep important content within ${safeArea}% safe area from edges.`, `AVOID: borders, frames, mockups, templates, extra limbs, distorted anatomy, low contrast, motion blur.`);
    return basePrompt.join(' ');
}
// Qwen image generation using qwen3imageedit endpoint logic
const generateWithQwen = async (params, prompt) => {
    try {
        ...
        const tryExtractImage = (obj) => {
            ...
            if (Array.isArray(msgContent)) {
                const imgItem = msgContent.find((c) => typeof c?.image === 'string');
                if (imgItem?.image)
                    return imgItem.image;
            }
            const outContent = obj?.output?.content;
            if (Array.isArray(outContent)) {
                const imgItem = outContent.find((c) => typeof c?.image === 'string');
                if (imgItem?.image)
                    return imgItem.image;
            }
            const result0 = obj?.output?.results?.[0];
            if (result0?.image_base64)
                return `data:image/png;base64,${result0.image_base64}`;
            if (typeof result0?.image === 'string')
                return result0.image;
            if (typeof result0?.output_image_url === 'string')
                return result0.output_image_url;
            return undefined;
        };
...
const generateWithOpenAI = async (params, prompt) => {
    try {
        ...
        const { toFile } = await Promise.resolve().then(() => __importStar(require('openai/uploads')));
        const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
        console.log(`🤖 OpenAI: Processing image data for job ${params.jobId}`);
        const dataUrl = params.originalImageBase64;
...
const generateBookTitle = (themeId, heroName) => {
...
const processImageGeneration = async (params) => {
    console.log(`Processing image generation for job: ${params.jobId}`);
    try {
        const { heroName, themeId, storyId, originalImageBase64, ageGroup, length, styleKey, kind, storyText, coverPromptOverride } = params;
        // Input validation
        if (!themeId) {
        ...
        const defaultScene = `${heroName} in a magical ${themeId} adventure`;
        const customScene = ne(coverPromptOverride) ?? defaultScene;
        // Build the prompt
                ...
                pages: [
                    {
                        ...
                        type: 'title',
                        title: bookTitle,
                        text: `A magical ${themeId} story featuring ${heroName}!`,
...
const processWithProgress = async (params, progressCallback) => {
    ...
    if (isMockMode) {
        ...
        const result = await (0, exports.processImageGeneration)(params);
